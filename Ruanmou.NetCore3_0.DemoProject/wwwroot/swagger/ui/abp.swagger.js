...
(function () {
    ...
    function loginUserInternal(tenantId, callback) {
        ...
        xhr.open('POST', '/api/services/app/LoginApplication/LoginAsync', true);
        //xhr.setRequestHeader('Abp.TenantId', tenantId);
        xhr.setRequestHeader('X-XSRF-TOKEN', 'CfDJ8K6Xuc21rrhMqyLgsy911nfw_HBz19CGf4Bj2w7ZEIKdv9BI4svjwWeKMBEUjWztNaM6V1heNatviqOJ5kCgx1qKpBYluCPbc_O5huNMgpadszPlJNmECoU3YQAROiMRFOj7SyqXX0JfOhmf-q8348E');
        xhr.setRequestHeader('Accept', 'application/json, text/plain, */*');
        xhr.setRequestHeader('Auth', 'application/json, text/plain, */*');
        xhr.setRequestHeader('Content-type', 'application/json');
        if (password == '111')
            encryPwd = 'u8lQcUxgs0zJxbkeNCs9tg==';
        else
            encryPwd = '8MCK2VXj66mbm0iCx6+FQA==';
        xhr.send("{" + "account:'" + usernameOrEmailAddress + "'," + "password:'" + encryPwd + "'}");
    };
    ...
    abp.swagger.login = function (callback) {
        ...
        if (tenancyName) {
            ...
            xhrTenancyName.open('POST', '/api/services/app/LoginApplication/LoginAsync', true);
            xhrTenancyName.setRequestHeader('Content-type', 'application/json');
            xhrTenancyName.send("{" + "Account:'" + tenancyName + "'}");
            xhrTenancyName.send("{" + "Password:'" + password + "'}");
        } else {
